...
-(void)changeToolandNavigation{
    NSTimeInterval  time = 0.3;
    if (!_isShowToolNavigation) {
        [UIView animateWithDuration:time animations:^{
            _toolbar.top +=_toolbar.height;
            _navigationView.top-=_navigationView.height;
        } completion:^(BOOL finished) {
            _isShowToolNavigation = !_isShowToolNavigation;
        }];
    }else{
        [UIView animateWithDuration:time animations:^{
            _toolbar.top -=_toolbar.height;
