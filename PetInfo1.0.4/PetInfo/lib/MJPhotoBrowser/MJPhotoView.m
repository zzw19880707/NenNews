...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:frame])) {
        ...
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(backAction) name:kImageReturnNofication object:nil];
...
- (void)handleSingleTap:(UITapGestureRecognizer *)tap {
    [self.photoViewDelegate changeToolandNavigation];
}
- (void)hide
{
...
- (void)handleDoubleTap:(UITapGestureRecognizer *)tap {
	...
	if (self.zoomScale >= self.maximumZoomScale) {
		...
		[self zoomToRect:CGRectMake(touchPoint.x-40, touchPoint.y-40, 80, 80) animated:YES];
	}
}
